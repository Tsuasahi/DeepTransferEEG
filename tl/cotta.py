...
def train_target(args):
    ...
    print('executing ' + str(args.type).upper() + '...')

    if args.balanced:
        acc_t_te, y_pred = CoTTA_func(dset_loaders["Target-Online"], base_network, args=args, balanced=True)
        log_str = 'Task: {}, {} Acc = {:.2f}%'.format(args.task_str, str(args.type).upper(), acc_t_te)
    else:
        acc_t_te, y_pred = CoTTA_func(dset_loaders["Target-Online-Imbalanced"], base_network, args=args, balanced=False)
        log_str = 'Task: {}, {} AUC = {:.2f}%'.format(args.task_str, str(args.type).upper(), acc_t_te)
    args.log.record(log_str)
    print(log_str)
    ...
    for data_name in data_name_list:
        ...
        for s in [1]:
            ...
            if args.type == 'ctta':
                ...
                # force set test batch and stride to 1 for CTTA
